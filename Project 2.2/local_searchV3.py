...
def search(input_dict):
    ...
    while best_fitness > 0:
        ...
        while improving:
            ...
            for i, j in itertools.product(range(len(above_target)), range(len(below_target))):
                for x in range(size):
                    for y in range(size):
                        value_above, value_below = above_target[i][0][x], below_target[j][0][y]
                        ...
                        new_fitness += (new_above_sum - target_sum) ** 2 + (new_below_sum - target_sum) ** 2

                        if new_fitness < current_fitness:
                            above_target[i][0][x], below_target[j][0][y] = value_below, value_above
