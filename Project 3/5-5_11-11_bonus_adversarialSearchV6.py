...
def quiescence_search(state: State, alpha: int, beta: int, maximizing: bool) -> int:
    ...
    for move in moves:
        ...
        new_state = State(new_board, "black" if maximizing else "white")
        score = -quiescence_search(new_state, -beta, -alpha, not maximizing)
...
def alpha_beta(
    state: State, depth: int, alpha: int, beta: int, maximizing: bool
    ...
    if opponent_king_position and any(
        ...
        return float("inf") if maximizing else float("-inf"), next(
            move
            for move in state.get_legal_moves()
    ...
    if depth == 0:
        return quiescence_search(state, alpha, beta, maximizing), None
